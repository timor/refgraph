...
Further processing ([[parse_bibtex_file]]) is initiated from its =onload= handler.
...
*** BibTex Parser
 ...
 #+BEGIN_SRC text :noweb-ref nil
   ...
   normal_entry = "@" type:name _ '{' _ key:key _ "," fields:(field_with_separator)* _ last_field:field? _ '}'
     { fields.push(last_field);
       var field_obj = {};
       fields.forEach(function(f) { if (!field_obj[f.name]) field_obj[f.name] = f.value});
     return {type: type, key: key, fields: field_obj} }
 ...
 The generated parser lives in [[file:scripts/parser.js][scripts/parser.js]] as =bibtex_parser=.  The main parser
 entry function is =bibtex_parser.parse=.
...
- should probably also parse the author names with PEG
