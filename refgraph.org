...
#+BEGIN_SRC css
  ...
  circle.paper.started {
      fill:orange;
  }

  circle.paper.overview {
      fill:yellow;
  }
...
The content of the tool tip is dependent on wether it is a paper or
author node.
...
**** Analyze BibTex file
   ...
   function analyze_bibtex_entries(entries) {
       ...
       entries.forEach(function(e) {
           if (!e.author) {
               ...
               return;}
           as = e.author.split(" and ").map(normalize_author).map(find_author);
           e.author = as; //replace author list with a list of author objects
           e.type = "paper";
           e.name = e.key;
           e.author.forEach(function(author) {
 ...
 sometimes only by short name.  Note that this possibly in
